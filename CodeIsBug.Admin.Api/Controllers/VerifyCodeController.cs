...
namespace CodeIsBug.Admin.Api.Controllers
{
    ...
    public class VerifyCodeController : ControllerBase
    {
        ...
        public async Task<Result> VerifyCode([FromBody] CkeckCodeDto checkCodeDto)
        {
            ...
            {
                throw new ArgumentNullException(nameof(checkCodeDto));
            }

            if (Equals(Guid.Empty, checkCodeDto.CheckGuid))
            {
                return new Result { Code = -1, Message = "参数错误" };
            }
            var code = await _redisHelper.GetValueAsync(checkCodeDto.CheckGuid.ToString());
            if (!code.HasValue)
            {
                return new Result { Code = 0, Message = "验证码已过期，请刷新页面重新尝试登录" };
            }
            if (!code.ToString().Equals(checkCodeDto.CheckCode))
            {
                return new Result { Code = 0, Message = "验证不通过" };
            }
