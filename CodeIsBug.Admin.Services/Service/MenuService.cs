...
namespace CodeIsBug.Admin.Services.Service
{
    public class MenuService : BaseService<ESysMenu>
    {
        ...
        {
            var list = await Context.Queryable<ESysMenu>().Where(a => a.Level == 0)
                .Select(a => new MenuFirstLevelInfo {MenuId = a.MenuId, Name = a.Name}).ToListAsync();
            return list;
        }
        ...
        {
            return await Context.Deleteable<ESysMenu>().Where(a => a.MenuId.Equals(menuId))
                .ExecuteCommandHasChangeAsync();
        }
    }
}